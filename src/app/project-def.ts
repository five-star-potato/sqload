...
export class ProjectStruct implements Serializable<ProjectStruct> {
    ...
    reduceOutputMappingRefCount(outMap: OutputMap) {
        ...
        if (outMap.refCount <= 0) {
            let i = this.outputMaps.findIndex(m => m.id == outMap.id);
            this.outputMaps.splice(i, 1);
        }
    }
    private getGroup(grpOrId: number | GroupDef): GroupDef {
        let grp:GroupDef;
        if (typeof(grpOrId) == "number")
            grp = this.groups.find(g => g.id == grpOrId);
        else 
            grp = grpOrId;  
        return grp;
    }
    joinGroups(targetGrpOrId: number | GroupDef, srcGrpOrId: number | GroupDef) {
        ...
        let srcIndex = this.groups.findIndex(g => g.id == srcGrp.id);
        this.groups.splice(srcIndex, 1);
    }
    ...
    ungroup(grp:GroupDef) {
        ...
        let grpIndex = this.groups.findIndex(g => g.id == grp.id);
    ...
    findEdgeObjInGroup(grpId: number, minmax: string = "min"):DBObjDef {
        let grp:GroupDef = this.groups.find(g => g.id == grpId);
        let dbObj:DBObjDef;
        this.forEachGroupMember(grp, a => {
            if (dbObj) {
                if (minmax == "min") {
    ...
    public isFirstObjInGroup(obj:DBObjDef) {
        ...
        let grp = this.groups.find(g => g.id == obj.groupId);
    ...
    public duplicateObj(obj: DBObjDef):DBObjDef {
        ...
        this.selectedObjs[obj.objType].push(newObj);
    ...
    public deleteObj(obj:DBObjDef) {
        ...
        let index = this.selectedObjs[obj.objType].findIndex(o => o.id == obj.id && o.instance == obj.instance);
        let cols = this.getAllColumnsByObj(obj);
        cols.forEach(c => {
            // remove outputmapping if any
            if (c.plugIn.length > 0 && c.plugIn[0].constructor.name == "CommandOutputGenerator") {
                let cmdGen = (c.plugIn[0] as gen.CommandOutputGenerator);
                if (cmdGen.outputMappingId) {
                    let outMap = this.outputMaps.find(o => o.id == cmdGen.outputMappingId);
                    this.reduceOutputMappingRefCount(outMap);
                }
            }
        });
        this.selectedObjs[obj.objType].splice(index, 1);
    }
