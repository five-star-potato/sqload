        ...
        tables.forEach((tbl:any) => {
            ...
            colArr.forEach((cf:ColumnDef) => {
                if (cf.include) {
                    ...
                    stmts.push(`DECLARE @T_${varRoot} TABLE ( value: ${fnGetDataTypeDesc(cf)} );\n`); // not all columns need this; so far only the ones that use CustomSqlGenerator needs it
                ...
                colArr.forEach((cf:ColumnDef) => {
                    if (cf.include) {
                        if (cf.plugIn.length == 0 && !cf.fkConstraintID) {
                            ...
                            else if (cf.plugIn[0] instanceof CustomSqlGenerator) {
